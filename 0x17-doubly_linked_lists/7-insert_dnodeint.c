...
dlistint_t *insert_dnodeint_at_index(dlistint_t **h, unsigned int idx, int n)
{
	...
	if (new_node == NULL || idx > list_len || (list_len == 0 && idx > 0))
		return (NULL);
	new_node->n = n;
	new_node->prev = NULL;
	new_node->next = NULL;
	if (list_len == 0) /* if list empty */
	{
		*h = new_node;
		return (new_node);
	}
	if (idx == 0) /* beginning of list */
	{
		new_node->next = *h;
		(*h)->prev = new_node;
	}
	if (idx == list_len) /* end of list */
	{
		while ((*h)->next != NULL)
			*h = (*h)->next;
		(*h)->next = new_node;
	...
	if (idx > 0 && idx < list_len) /* middle of list */
	{
		while (ch < (idx - 1))
		{
			*h = (*h)->next;
